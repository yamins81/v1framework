...
def ext_eval_ht_l2_gabor_and_random_fav(depends_on=('../config/fav_task.py',
                                                  ...
                                                  '../config/ten_categories_images.py')):
    """
    hash : e98e12ddf1c9063b52bfc5ebca20762a36ca0edf
    level 1, filter, model_name
    value: [u'gridded_gabor', u'really_random']
    max: 63.60, 62.40
    mean: 57.19, 54.61
    quartiles 57.07, 54.30
    
    So having gabors helps a bit
    """
    a,b,c = depends_on
    protocols.extract_and_evaluate_protocol(a,b,c,convolve_func_name='numpy', write=True, parallel='semi')
...
#these experiments are a bit flawed ... due to errors in the setup of the images
#so I'm not sure what to make of the conclusions
...
def ext_eval_ht_l2_gabor_random_o2_top5_pairwise_renderman(depends_on=('../config/renderman_pairwise_tasks.py',
    ...
    bottom line: There's a lot of variation in two-way tasks in terms of 
    difference between translation and rotation, and mixed/control tasks.
    Moreover, this variation is generated by particular categories, e.g.
    some categories (like faces) are easy to separate while others 
    (like animals and planes) screw up alot of the pairwise comparisons.
    Overall rotation is much harder than translation.
    
    So therefore some of these tasks might be worth studying for understanding
    where our algorithms are going wrong.
    """
    a,b,c = depends_on
    protocols.extract_and_evaluate_protocol(a,b,c,convolve_func_name='numpy', write=True, parallel='semi')

@protocolize()
def ext_eval_ht_l2_gabor_random_o2_top5_pairwise_renderman_backgrounds(depends_on=('../config/renderman_pairwise_tasks_background.py',
                                                  '../config/ht_l2_gabor_random_o2_top5_renderman_models.py',
                                                  '../config/ten_categories_images.py')):
    """
    #documented in nugget 2
    bottom line: some pairwise tasks get worse due to background, some don't much.  T-I and T-M are always positive.
    """
    a,b,c = depends_on
    protocols.extract_and_evaluate_protocol(a,b,c,convolve_func_name='numpy', write=True, parallel='semi')
...
#and using feed up 
#bottom line: can add alot, but obviously need to explore alot (e.g. number of filters and relative sizes)
...
@protocolize()
def make_lt_l2_gabor_gabor_models4(depends_on='../config/lt_l2_gabor_gabor_models4.py'):
    """
    """
    protocols.model_protocol(depends_on,parallel=False,write=True)    
...
def ext_eval_lt_l2_gabor_gabor_models4_renderman(depends_on=('../config/renderman_tasks_for_ht_overlap.py',
                                                  '../config/lt_l2_gabor_gabor_models4.py',
                                                  '../config/ten_categories_images.py')):
    """
    """
    a,b,c = depends_on
