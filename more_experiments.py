...
def ext_eval_ht_l2_gabor_random_polygon(depends_on=('../config/parallel_polygon_tasks_for_ht.py',
    ...
    with 256 l2 filters:
    	92.20, 55.00, 80.48, 86.48, 8.56
    with 384 l2 filters: 
    	93.80, 57.20, 83.17, 88.50, 8.30
...
def ext_eval_l2_freq_test_3_polygon(depends_on=('../config/parallel_polygon_tasks_for_ht.py',
                                                  ...
                                                  '../config/polygon_task.py')):
    """
    taking best l2 freq from highthroughput run and changing l1 pool order, performance climbs almost 10% to 88.8%
...
def ext_eval_l2_random_random_test_polygon(depends_on=('../config/parallel_polygon_tasks_for_ht.py',
                                                  ...
                                                  '../config/polygon_task.py')):
    """
    seeing if changing l1pool order to 2 drops performance (yes, by 5% to 79%)
...
def ext_eval_l1_gabor_test_2_polygon_2(depends_on=('../config/polygon_tasks_2.py',
                                                  ...
                                                  '../config/polygon_task.py')):
    """
    seeing if doubling the training examples pushes performance up alot
    result: a bit: performance up to 89.7 for 80/per class and 92.6 for 160 / class
...
def ext_eval_l1_gabor_test_3_polygon(depends_on=('../config/parallel_polygon_tasks_for_ht.py',
                                                  ...
                                                  '../config/polygon_task.py')):
    """
    seeing if adding even more filters (16 orients x 7 freqs) helps ... not really
...
@protocolize()
def make_ht_l2_gabor_gabor_models(depends_on='../config/ht_l2_gabor_gabor_models.py'):
    """
...
@protocolize()
def ext_eval_ht_l2_gabor_gabor_renderman(depends_on=('../config/renderman_tasks_for_ht.py',
                                                  '../config/ht_l2_gabor_gabor_models.py',
                                                  '../config/ten_categories_images.py')):
    """
